...
def fetch_data(start_time: datetime, length: dt.timedelta, camera_id: str) -> List[DetectionRow]:
    conn_params = {
        'dbname': 'saebackend',
        'user': 'saebackend',
        'password': 'saebackend',
    ...
    try:
        with psycopg.connect(**conn_params) as conn:
            with conn.cursor() as cur:
                query = "SELECT * FROM detection WHERE capture_ts >= %s and capture_ts <= %s and camera_id = %s;"
                cur.execute(query, (start_time.isoformat(), start_time + length, camera_id))
                rows = cur.fetchall()
                return rows
...
def draw_line(img, pt1: Tuple[float], pt2: Tuple[float], class_id: int):
    cv2.line(img, (int(pt1[0] * img.shape[1]), int(pt1[1] * img.shape[0])), (int(pt2[0] * img.shape[1]), int(pt2[1] * img.shape[0])), color=get_color(class_id), thickness=2)

if __name__ == "__main__":
    arg_parser = argparse.ArgumentParser()
    arg_parser.add_argument('-c', '--camera-id', type=str, required=True)
    arg_parser.add_argument('-i', '--image-file', type=Path)
    arg_parser.add_argument('-s', '--start-time', type=dt.datetime.fromisoformat, default=datetime.now() - dt.timedelta(hours=1), help='Data segment start time (in isoformat)')
    ...
    output_file = image_file.parent / f'{args.camera_id}_{args.start_time}_{args.length.seconds}s.jpg'

    print('Loading data...')

    rows = fetch_data(args.start_time, args.length, args.camera_id)
